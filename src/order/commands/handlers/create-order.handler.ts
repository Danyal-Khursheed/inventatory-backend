  ...
  async execute(command: CreateOrderCommand) {
    try {
      ...
      const order = this.orderRepo.create({
        ...
        receiverCompanyName: dto.receiver.companyName ?? null,
        receiverEmail: dto.receiver.email ?? null,
        receiverMobileNo: dto.receiver.mobileNo,
        cod: dto.cod ?? false,
        referenceId: dto.referenceId ?? null,
        codAmount: dto.codAmount ?? null,
        instructions: dto.instructions ?? null,
        boxLength: dto.box?.length ?? null,
        boxWidth: dto.box?.width ?? null,
        boxHeight: dto.box?.height ?? null,
        volumetricWeight: dto.box?.volumetricWeight ?? null,
        quantityOrdered: totalQuantity,
        unitPrice: totalQuantity ? totalPrice / totalQuantity : 0,
        totalPrice,
        totalWeight,
        orderStatus: 'pending',
